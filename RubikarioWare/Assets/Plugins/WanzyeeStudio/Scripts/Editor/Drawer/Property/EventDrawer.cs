...
#pragma warning restore CS0253 // Possibilité d'une comparaison de références involontaire ; la partie droite a besoin d'un cast
		...
		private void Initialize(SerializedProperty property){
			...
			try{
				...
				AutoAssignTargetUnityEvent(_list);
			}
			catch{}
		...
		/// <summary>
		/// Auto Assign Current Target
		/// <br></br>Neat Addition by Ewan Argouse :)
		/// </summary>
		/// <param name="reorderableList"></param>
		private void AutoAssignTargetUnityEvent(ReorderableList reorderableList)
		{
			reorderableList.onAddCallback += (ReorderableList list) =>
			{
				SerializedProperty GetTargetUnityEvent(SerializedProperty sp)
				=> sp.GetArrayElementAtIndex(list.index).FindPropertyRelative("m_Target");

				Object GetTarget(SerializedProperty sp)
				=> sp.serializedObject.targetObject;

				GetTargetUnityEvent(list.serializedProperty).objectReferenceValue = GetTarget(list.serializedProperty);
			};
		}
